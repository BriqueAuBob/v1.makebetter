...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(`<div${ssrRenderAttrs(vue_cjs_prod.mergeProps({ class: "min-h-screen grid md:grid-cols-4 lg:grid-cols-5" }, _attrs))}><div class="col-span-3 md:col-span-3 lg:col-span-2 flex flex-col justify-center items-center gap-8 container mx-auto px-4 lg:px-16 max-w-xl py-48"><div class="w-full"><h1 class="font-semibold text-xl">Authentification</h1><h2 class="font-medium text-md mt-1">Tu peux te connecter via diff\xE9rentes m\xE9thodes !</h2></div><div class="w-full flex flex-col gap-4">`);
  _push(ssrRenderComponent(_component_Button, {
    ...
    icon_left: vue_cjs_prod.withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`<img${ssrRenderAttr("src", _imports_0)} alt="Discord" class="h-6 w-6 mr-4"${_scopeId}>`);
      } else {
        return [
  ...
  _push(`</div><div class="w-full"><div class="relative flex items-center"><div class="flex-grow border-t border-dark-700"></div><span class="flex-shrink mx-4 text-dark-700">or</span><div class="flex-grow border-t border-dark-700"></div></div></div><div class="w-full flex flex-col gap-6 blur-sm">`);
  ...
  _push(ssrRenderComponent(_component_NuxtLink, {
    to: "#",
    class: "block font-medium text-primary-500 mt-3 underline blur-sm"
  }, {
  ...
  _push(`</div></div></div><div class="bg-primary-500 lg:col-span-3 relative"><img${ssrRenderAttr("src", _imports_1)} class="absolute left-1/2 -translate-x-1/2 bottom-0 w-3/4" alt="Group characters"></div></div>`);
}
