<template>
    <div v-if="module">
        ...
        <section class="relative bg-white dark:bg-dark-800 py-12 shadow-sm dark:shadow-lg">
            <div class="absolute bg-white dark:bg-dark-700 p-12 rounded-3xl shadow-2xl left-1/2 -translate-x-1/2 top-4 -translate-y-2/3 flex gap-4 lg:gap-12 font-extrabold text-5xl dark:text-white text-black items-center w-fit z-10">
            ...
            <Slider direction="horizontal" duration="60s">
                <div class="flex">
                    <div v-for="i in 3" :key="i" class="flex">
                        <div v-for="(tool, i) in tools" :key="i" class="bg-gray-200 dark:bg-dark-700 text-gray-500 dark:text-gray-400 px-6 py-4 rounded-3xl font-semibold text-lg mr-6">
                            {{ getToolName(tool) }}
                        </div>
                    </div>
                </div>
        ...
        <section class="bg-gradient-to-b from-primary-400 to-primary-500 py-24 text-white mt-8">
...
export default {
    ...
    methods: {
        ...
        getModuleCard(index) {
            return this.cards[this.getModuleName(index)]
        }
    }
}
</script>